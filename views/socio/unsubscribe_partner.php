<?php
    ...
    $namePartner = strtoupper($socio['paterno']." ".$socio['materno']." ".$socio['nombres']);
    $ciPartner = $socio['ci']." ".$socio['expedido'];
    ...
    function literalMonth($month){
        ...
        return isset($literal[$month]) ? $literal[$month] : 'S/M';
    }
...
<body>
    ...
    <div id="watermark">
        <div class="row justify-content-center">
            <div class="col-lg-12 text-center">
                <img src="data:image/jpeg;base64,<?= $document['watermark'] ?>"/>
            </div>
        </div>
    </div>
    ...
    <div class="row mt-2"  style="font-size: 15px;">
        <div class="col-lg-12 text-right">
            ...
            La Paz, <?= $fecha[2] ?> de <?= literalMonth($fecha[1]) ?> de <?= $fecha[0] ?>
        </div>
    </div>
    ...
    <div class="row" style="font-size: 16px;">
        <div class="col-12 text-justify">
            ...
            <p class="mt-2">
                Que el Señor <b><?= $namePartner ?></b>, con Cédula de Identidad N° <?= $ciPartner ?>, solicita la devolución de sus aportes con incremento, realizados al Círculo de Oficiales Navales "Stella Maris" adjuntando la documentación conforme Normativa Vigente.
            </p>
            ...
            <br>
                <?php $fecha = ($socio['fechaIncorporacion'] == null ? ['','',''] : explode("-",$socio['fechaIncorporacion'])); ?>
                Que el Señor <b><?= $namePartner ?></b> inicio sus aportes al Círculo de Oficiales Navales “Stella Maris”, en el mes de <?= literalMonth($fecha[1]) ?> de <?= $fecha[0] ?> y cumplió su aporte número <?= $numAportes ?> en el mes de <?= literalMonth($lastMonthContribution) ?> de <?= $lastYearContribution ?>.
            <br>
            ...
            <p class="mt-2">
                <b>PRIMERO:</b> Aprobar la Devolución de aportes con incremento al Señor <b><?= $namePartner ?></b>, del periodo <?= literalMonth($firstMonthContribution) ?> de <?= $firstYearContribution ?> a <?= literalMonth($lastMonthContribution) ?> de <?= $lastYearContribution ?>, conforme el siguiente cuadro:
            </p>
        </div>
    </div>
        ...
        <div class="col-lg-12">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td align="right"><?= number_format($fullRefund, 2) ?></td>
                </tr>
            </table>
        </div>
    </div>
    <div class="row mt-4 text-justify" style="font-size: 15px;">
        <div class="col-lg-12">
            <p>
                <b>SEGUNDO:</b> Autorizar a la Dirección Financiera del Círculo de Oficiales Navales “Stella Maris”, el depósito de los aportes más el incremento al Señor <b><?= $namePartner ?></b> a su cuenta individual en el Banco Unión S.A., monto que asciende a Bs. <?= number_format($fullRefund, 2) ?> (<?= numtoletras(round($fullRefund, 2)) ?> BOLIVIANOS).
            </p>
            <p>
    ...
    <div class="row text-center">
        <div class="col-lg-12" style="line-height:1;margin-top:70px;">
            <?= $socio['paterno']." ".$socio['materno']." ".$socio['nombres'] ?><br>
            <b><?= $ciPartner ?></b>
        </div>
    </div>
    <div class="row text-center">
